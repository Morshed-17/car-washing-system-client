...
export function Navbar() {
  ...
  const nextBooking = bookings
    ?.filter((booking) => new Date(`${booking.slot.date}T${booking.slot.startTime}`) > new Date())
    ?.sort(
      (a, b) =>
  ...
  const targetDateTime = nextBooking ? `${nextBooking.slot.date}T${nextBooking.slot.startTime}:00` : null
    ...
    >
      ...
      {targetDateTime && (
        <div className="text-sm font-medium container flex items-center justify-center py-3 px-4 mx-auto border-b gap-1">
          ...
          <CountdownTimer targetDateTime={targetDateTime} className="text-primary" />
        </div>
      )}
